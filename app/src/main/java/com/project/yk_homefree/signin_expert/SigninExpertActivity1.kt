...
class SigninExpertActivity1 : AppCompatActivity() {

    private var binding: ActivitySigninExpert1Binding? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivitySigninExpert1Binding.inflate(layoutInflater)
    ...
    private fun checking() {
        ...
        if (binding?.check1?.isChecked == true && binding?.check2?.isChecked == true && binding?.check3?.isChecked == true) {
            binding?.signinExpert1NextBtn?.isClickable = true
        } else {
            return
        }
    }
    ...
    private fun nextBtnAction() {
        binding?.signinExpert1NextBtn?.setOnClickListener {
            if (binding?.check1?.isChecked == true && binding?.check2?.isChecked == true && binding?.check3?.isChecked == true) {
                val intent = Intent(this, SigninExpertActivity2::class.java)
